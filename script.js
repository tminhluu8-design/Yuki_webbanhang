...
function bamMoOrder() {
    let giay = 1;
    const nutBam = document.getElementById('btnOrder');
    nutBam.disabled = true; // Khóa nút không cho bấm liên tục
	order();
    // Chạy đồng hồ đếm ngược mỗi giây
    const demNguoc = setInterval(() => {
        nutBam.innerText = `Vui lòng đợi... (${giay}s)`;
        giay--;
        
        if (giay < 0) {
            clearInterval(demNguoc); // Dừng đếm ngược
            nutBam.innerText = "Đang chuyển trang...";
            //window.location.href = 'dathang.html'; // Mở file HTML
			window.open('dathang.html', '_blank');
			//nutBam.innerText = "Done chuyển trang...";
        }
    }, 1000);
}
...
function order() {
	//const textareaNote_dathang = document.getElementById('textareaNote');
	//textareaNote_dathang.innerMessage = "try_to_put_Text";
    if(cart.length === 0) {
        alert("Giỏ hàng của bạn đang trống!");
        return;
    }
    //alert("Cảm ơn bạn đã đặt hàng! Nhân viên sẽ kiểm tra thông tin và liên hệ với bạn qua phone hoặc email.");	
	cart = []; // Xóa sạch giỏ hàng sau khi đặt thành công
    updateCartUI();
    toggleCart(); // Đóng modal giỏ hàng
	// Thay đổi địa chỉ URL của tab hiện tại sang contact.html
